...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h4">Kategori Produk</h1>
</div>
...
<div class="klik fs-6 mb-2">Tambah Kategori
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="xmodalLabel">Tambah Kategori</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="/kategori" class="form-container p-2" method="post">
            @csrf
        ...
        @foreach ($kategori as $kt)
          <tr id="{{ $kt->id }}">
            ...
            <td style="text-align: center">
              <a href="/kategori/{{ $kt->id }}/edit" class="btn btn-sm btn-info"><i data-feather="edit"></i><a>
              @csrf
              |
              <form action="/kategori/{{ $kt->id }}" method="post" class="d-inline">
               @csrf
               @method('delete')
...
{{-- <div class="modal fade" id="formedit" tabindex="-1" aria-labelledby="xmodalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="xmodalLabel">Edit Kategori</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="/postkategori/{{ $kt->id }}" id="formedit" class="form-container p-2" method="post">
            @csrf
            <div class="form-floating mb-3">
              <input type="text" name="nama" class="form-control rounded @error ('nama') is-invalid @enderror" id="nama" placeholder="Nama" required autofocus value="{{ old('nama', $kt->nama) }}">
              <label for="nama">Nama</label>
              @error ('nama')
                <div class="invalid-feedback">
                  {{ $message }}
